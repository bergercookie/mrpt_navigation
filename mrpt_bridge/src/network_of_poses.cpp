...
bool convert(
	...
	// fill the constraints -- same as in the conversion from CNetworkOfPoses2DInf
	for (CNetworkOfPoses2DInf::const_iterator constr_it = constraints.begin();
			constr_it != constraints.end();
			++constr_it) {
		mrpt_msgs::GraphConstraint ros_constr;
...
bool convert(
	...
	for (nodes_cit_t nodes_cit = ros_graph.nodes.vec.begin();
			nodes_cit != ros_graph.nodes.vec.end();
			++nodes_cit) {
...
bool convert(
	...
	typedef mrpt::graphs::CNetworkOfPoses2DInf_NA::global_pose_t mrpt_graph_pose_t;
